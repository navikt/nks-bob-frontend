import { BodyShort, Heading, Label, Link, ReadMore, VStack } from "@navikt/ds-react"
  ...
  return (
    <ReadMore header="Kilder" className="mt-4">
      <VStack className="py-2">
        <BodyShort>
          Denne oversikten viser alle kildene Bob har sett igjennom for å lage svaret.
        </BodyShort>
        <VStack gap="4">
          <VStack gap="3">
            ...
            {nksContext.map((ctx) => <NksSource context={ctx} />)}
          </VStack>
          <VStack gap="3">
            <Heading level="2" size="xsmall">
              Nav.no
            </Heading>
            {navContext.map((ctx) => <NavSource context={ctx} />)}
          </VStack>
        </VStack>
      </VStack>
    </ReadMore >
