...
import { useAddIgnoredWord, useAlerts, useUserInfo } from "../../lib/api/api.ts"
...
import {
  ...
  ValidationWarning,
  Validator,
} from "../../lib/utils/validation/inputValidation.ts"
...
const NewMessageAlert = ({ setInputValue, conversationId }: NewMessageAlertProps) => {
  ...
  const startNew = () => {
    ...
    setInputValue("")
    if (conversationId) {
      previousConversationStore.getState().set(conversationId as string)
    }
    router.push("/")
  }
